...
public class CreateConstraintXML {
	...
	// constructor
	public CreateConstraintXML(RepositoryList CRList) {
		loadModel();
		// System.out.println(Configuration.FilePathConstraintTemplate);
		createconstraints(CRList);
	}
...
//			e.getMessage();
	...
	public int createconstraints(RepositoryList CRList) {
		if (!loadrez)
			return 1;
		...
		try {
			...
			if (Configuration.Utilization < 1)
				for (int i = 0; i < CRList.ContainerList.ncount; i++) {
					...
					Element hasMaxValue = maindoc.createElement("hasMaxValue");
					int g = (int) Math.round(Configuration.Utilization * 100);
					String a = Integer.toString(g);
					hasMaxValue.setTextContent(a);
					range.appendChild(hasMaxValue);
				}
			for (int i = 0; i < CRList.ContainerList.ncount; i++)
				if (Configuration.MemoryDemand[i] > 0) {
					...
					String a = Integer.toString(Configuration.MemoryDemand[i]);
					hasMaxValue.setTextContent(a);
					range.appendChild(hasMaxValue);
				}
